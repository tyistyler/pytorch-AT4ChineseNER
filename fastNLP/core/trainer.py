...
class Trainer(object):
    ...
    def __init__(self, train_data, cws_data,
                 model, optimizer=None, cws_optimizer=None, loss=None, cws_losser=None,joint_losser=None,
        ...
        :param use_knowledge
        :param train_feature_data list: [[{word}, {word}]]
        :param test_feature_datalist: [[{word}, {word}]]
        :param feature2count
        :param feature2id
        :param id2feature
        """
        ...
        # train_data: <fastNLP.core.dataset.DataSet>
        if isinstance(train_data, DataSet):
            self.data_iterator = DataSetIter(
                dataset=train_data, batch_size=batch_size, num_workers=num_workers, sampler=sampler, drop_last=drop_last)
            self.cws_data_iterator = DataSetIter(
                dataset=cws_data, batch_size=batch_size, num_workers=num_workers, sampler=sampler, drop_last=drop_last)
        elif isinstance(train_data, BatchIter):
            self.data_iterator = train_data
            train_data = train_data.dataset
            self.cws_data_iterator = cws_data
            cws_data = cws_data.dataset
        else:
        ...
        self.cws_data = cws_data
        self.dev_data = dev_data  # If None, No validation.
        self.losser = losser
        self.cws_losser = cws_losser
        self.joint_losser = joint_losser
        ...
        if self.dev_data is not None:
            self.tester = Tester(model=self.model,
                                 ...
                                 args=self.args,
                                 gram2id=self.gram2id,
                                 dataset = 'dev'
                                 # n_device=self.device
                                 )
    ...
    def train(self, load_best_model=True, on_exception='auto'):
        ...
        for index in indices:
            input_ids, input_mask, _, _, ngram_ids, ngram_positions, _, _, ngram_masks, _, _ = self.zen_train_dataset[index]
    ...
    def _train(self):
        ...
        with inner_tqdm(total=self.n_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True) as pbar:
            ...
            cws_data_iterator = self.cws_data_iterator
            ...
            for epoch in range(1, self.n_epochs + 1):
                ...
                for ner_batch, cws_batch in zip(data_iterator, cws_data_iterator):
                    indices = ner_batch[0]
                    batch_x = ner_batch[1]
                    batch_y = ner_batch[2]

                    cws_indices = cws_batch[0]
                    cws_batch_x = cws_batch[1]
                    cws_batch_y = cws_batch[2]   
                    ...
                    for joint_i in range(2):
                        if joint_i == 0:    # NER
                            prediction = self._data_forward(self.model,is_ner=True, batch_x=batch_x, cws_batch_x=cws_batch_x)
                            ...
                            #    print(batch_y)
                            ner_loss = self._compute_loss(prediction, batch_y).mean()   # 默认使用'loss'，而不是'cws_loss'
                            joint_loss = self.joint_losser(prediction, cws_batch_y).mean()
                            loss = ner_loss + 0.06 * joint_loss
                            ...
                            prediction = self._data_forward(self.model, is_ner=False, batch_x=batch_x,
                                                                cws_batch_x=cws_batch_x)
                            ...
                            cws_loss = self.cws_losser(prediction, batch_y).mean()  # 默认使用'loss'，而不是'cws_loss'
                            joint_loss = self.joint_losser(prediction, cws_batch_y).mean()
                            loss = cws_loss + 0.06 * joint_loss

                            # print('cws_loss:', cws_loss)
                            # print('ner_all+joint_loss:', loss)
                            '''
                                loss使用的是LossInForward，以下两者相等
                                ner_loss = self._compute_loss(prediction, batch_y).mean()
                                ner_loss = prediction['loss'].mean())
                            '''
                    ...
                    if self.step % self.print_every == 0:
                        ...
                        # self.logger_func("[epoch: {:>3} step: {:>4}] train loss: {:>4.6} time: {}".format(
                        #         epoch, self.step, avg_loss, diff))
    ...
    def _data_forward(self, network, is_ner, batch_x, cws_batch_x):
        # x = _build_args(self._forward_func, **x)
        y = network(
            is_ner,
            batch_x.get("chars"),
            cws_batch_x.get("chars").to(self.device),
            batch_x.get("target"),
            cws_batch_x.get("target"),
